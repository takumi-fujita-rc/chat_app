...
このフローを中心に、62個のLabelが以下の11カテゴリに分類されています。
...
```
Initiative Issue (hierarchy:initiative) ─ 複数Epicを束ねる最上位
    │
...
| 7 | **SPECIAL** | 8 | 特殊操作 | `🔐 security`, `💰 cost-watch`, `🏛️ initiative` |
...
| 11 | **HIERARCHY** | 5 | Issue階層 | `🏛️ hierarchy:initiative`, `🌳 hierarchy:root`, `🍃 hierarchy:leaf` |
...
| `🏛️ initiative` | Initiative管理単位 | 複数Epicを束ねる最上位 |
...
### 11. HIERARCHY Labels (5個) - Issue階層

| ラベル | 意味 | 用途 |
|--------|------|------|
| `🏛️ hierarchy:initiative` | Initiative Issue | 複数Epicを束ねる最上位管理単位 |
